...
import pl.dch.creditassistant.knowledge.application.TextNormalizer;
...
/**
 * AC-005 ingestion with LangChain4j: normalization -> chunking -> embedding -> vector persistence.
 * Re-ingesting a document replaces its previously stored chunks, so repeated runs do not create duplicates.
 */
class LangChain4jKnowledgeIngestionService implements KnowledgeIngestionService {

    private static final Logger log = LoggerFactory.getLogger(LangChain4jKnowledgeIngestionService.class);
    ...
    LangChain4jKnowledgeIngestionService(
